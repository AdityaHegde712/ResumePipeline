...
:::tip[Gemini 3 Models]
For **Gemini 3+ models** (e.g., `gemini-3-pro-preview`), LiteLLM maps `reasoning_effort` to the `thinking_level` field instead of `thinking_budget` when you set it. Supported levels depend on the model (Flash-family models also support `minimal` and `medium`). If you omit `reasoning_effort`, LiteLLM does **not** send a default `thinking_level` â the request uses the **Gemini API defaults** (Gemini 3 Flash defaults to `high` on the API).
:::
...
| ---------------- | -------- | ----- |
| "none"           | "budget_tokens": 0, "includeThoughts": false | ð° **Recommended for cost optimization** - OpenAI-compatible, always 0 |
...
completion(
    ...
    messages=messages, 
    response_format={"type": "json_object", "response_schema": response_schema} # ð KEY CHANGE
    )
...
try: 
    ...
    response_format={
        ...
        "enforce_validation": true # ð KEY CHANGE
    }
	)
...
tools = [{"googleSearch": {}}] # ð ADD GOOGLE SEARCH
...
4. On subsequent turns, include the full assistant message in your conversation history â LiteLLM re-injects the server-side parts automatically
...
- Server-side tool invocations (`toolCall`/`toolResponse`) are **not** included in `tool_calls` â they are in `provider_specific_fields["server_side_tool_invocations"]` because they were already executed by Google, not by your code
...
tools = [{"codeExecution": {}}] # ð ADD GOOGLE SEARCH
...
completion(
    model="gemini/gemini-1.5-pro", 
    messages=messages, 
    response_format={"type": "json_object"} # ð KEY CHANGE
)
...
litellm.set_verbose = True # ð See Raw call 
    ...
    messages=[
        {
            ...
            "content": [
                {"type": "text", "text": "Please summarize the audio."},
                {
                    "type": "file",
                    "file": {
                        "file_data": "data:audio/mp3;base64,{}".format(encoded_data), # ð SET MIME_TYPE + DATA
                    }
                },
            ],
        }
    ...
    messages=[
        {
            ...
            "content": [
                {"type": "text", "text": "Please summarize the file."},
                {
                    "type": "file",
                    "file": {
                        "file_id": "https://storage...", # ð SET THE IMG URL
    ...
    messages=[
        {
            ...
            "content": [
                {"type": "text", "text": "Please summarize the file."},
                {
                    "type": "file",
                    "file": {
                        "file_id": "gs://storage...", # ð SET THE IMG URL
                        "format": "application/pdf" # OPTIONAL
                    }
                },
            ],
...
{
    {
        "role": "system",
        "content": ...,
        "cache_control": {"type": "ephemeral"} # ð KEY CHANGE
    },
    ...
}
```
...
for _ in range(2): 
    ...
    print(resp.usage) # ð 2nd usage block will be less, since cached tokens used
```
            ...
            "content": [
                {
                    ...
                    "cache_control": {
                        "type": "ephemeral",
                        "ttl": "3600s"  # ð This TTL will be ignored (first one is used)
                    },
                }
            ],
        }
